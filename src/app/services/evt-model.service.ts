...
export class EVTModelService {
  public readonly editionSource$: Observable<OriginalEncodingNodeType> =
    this.editionDataService.parsedEditionSource$.pipe(shareReplay(1));
  ...
  public readonly projectInfo$ = this.prefatoryMatterParser.projectInfo$.pipe(shareReplay(1));
  ...
  public readonly relations$ = this.parsedLists$.pipe(map(({ relations }) => relations));
  ...
  public readonly appVariance$ = this.witnesses$.pipe(
    switchMap(witList =>
      this.significantReadingsNumber$.pipe(
  ...
  public readonly specialChars$ = combineLatest([this.characters$, this.glyphs$]).pipe(
