...
export class CharParser extends EmptyParser implements Parser<XMLElement> {
  ...
  getName(xml: XMLElement) {
    ...
    const localPropName = Array.from(xml.querySelectorAll<XMLElement>('localProp')).find(
      el => getDefaultAttr(el.getAttribute('name')).toLowerCase() === 'name'
    );
  ...
  getEntityName(xml: XMLElement) {
    const localPropName = Array.from(xml.querySelectorAll<XMLElement>('localProp')).find(
      el => getDefaultAttr(el.getAttribute('name')).toLowerCase() === 'entity'
    );
  ...
  getMappings(xml: XMLElement): CharMapping[] {
    return Array.from(xml.querySelectorAll<XMLElement>('mapping')).map(el => ({
  ...
  getLocalProps(xml: XMLElement): CharProp[] {
    return Array.from(xml.querySelectorAll<XMLElement>('localProp')).map(el => ({
  ...
  graphicsParser(xml: XMLElement): Graphic[] {
    ...
    return Array.from(xml.querySelectorAll<XMLElement>('graphic')).map(el => graphicParser.parse(el));
  }
}
