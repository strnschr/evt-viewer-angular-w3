...
export class NamedEntitiesParserService {
  ...
  public parseLists(document: XMLElement) {
    ...
    const lists = (
      listsToParse.toString() ? Array.from(document.querySelectorAll<XMLElement>(listsToParse.toString())) : []
    )
  ...
  public parseNamedEntitiesOccurrences(pages: Page[]) {
    return pages
      .map(p => this.getNamedEntitiesOccurrencesInPage(p))
      .reduce((x, y) => {
  ...
  public getNamedEntitiesOccurrencesInPage(p: Page): Array<Map<NamedEntityOccurrence>> {
    return p.originalContent
      ...
      .map(e => {
        const occurrences = [];
        if (this.tagNamesMap.occurrences.indexOf(e.tagName) >= 0 && e.getAttribute('ref')) {
          // Handle first level page contents
          occurrences.push(this.parseNamedEntityOccurrence(e));
        }

        return occurrences.concat(
          Array.from(e.querySelectorAll<XMLElement>(this.tagNamesMap.occurrences)).map(el =>
            this.parseNamedEntityOccurrence(el)
          )
        );
      })
      .filter(e => e.length > 0)
