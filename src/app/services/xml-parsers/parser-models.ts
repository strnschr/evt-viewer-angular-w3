...
export type ParseResult<T extends GenericElement> =
  | T
  | HTML
  | GenericElement
  | Attributes
...
export interface Parser<T> {
  parse(data: T): ParseResult<GenericElement>;
}
...
export function createParser<U, T extends Parser<U>>(c: new (raw: ParseFn) => T, data: ParseFn): T {
  return new c(data);
}

export function getID(xml: XMLElement, prefix: string = '') {
  return xml.getAttribute('xml:id') || prefix + xpath(xml);
}
export function getClass(xml: XMLElement) {
  return xml.tagName ? xml.tagName.toLowerCase() : '';
}
...
export function getDefaultN(n: string) {
  return n || '';
}
export function getDefaultAttr(attr: string) {
  return attr || '';
}
