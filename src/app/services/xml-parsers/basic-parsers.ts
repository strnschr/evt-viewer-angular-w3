...
export class EmptyParser {
  ...
  constructor(parseFn: ParseFn) {
    this.genericParse = parseFn;
  }
}
...
export class NoteParser extends EmptyParser implements Parser<XMLElement> {
  ...
  parse(xml: XMLElement): Note {
    const noteLayout: NoteLayout =
      this.isFooterNote(xml) ||
      this.isNamedEntityNote(xml) ||
  ...
  private isFooterNote(xml: XMLElement) {
    return isNestedInElem(xml, 'div', [{ key: 'type', value: 'footer' }]);
  }
  private isNamedEntityNote(xml: XMLElement) {
    return isNestedInElem(xml, 'relation') || isNestedInElem(xml, 'event');
  }
}
