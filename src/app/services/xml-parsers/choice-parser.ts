...
export class ChoiceParser extends EmptyParser implements Parser<XMLElement> {
  ...
  private getEditorialInterventionType(xml: XMLElement): ChoiceType | '' {
    const sicCorEls = Array.from(xml.querySelectorAll<XMLElement>('sic, corr')).filter(el => el.parentElement === xml);
    if (sicCorEls.length > 0) {
      return 'emendation';
    }
    const origRegEls = Array.from(xml.querySelectorAll<XMLElement>('orig, reg, abbr, expan')).filter(
      el => el.parentElement === xml
    );
