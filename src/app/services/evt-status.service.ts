...
export class EVTStatusService {
  ...
  public currentPage$ = merge(
    merge(this.route.queryParams.pipe(map((params: URLParams) => params.p)), this.updatePageId$).pipe(
      mergeMap(id =>
        this.evtModelService.pages$.pipe(map(pages => (!id ? pages[0] : pages.find(p => p.id === id) || pages[0])))
      )
    ),
    this.updatePage$.pipe(filter(p => !!p)),
  ...
  public currentStatus$: Observable<AppStatus> = combineLatest([
    ...
    shareReplay(1),
    map(([viewMode, document, page, editionLevels, witnesses, versions]) => {
