...
export class NavBarComponent {
  ...
  thContainerInfo$ = this.updateThContainerInfo$.pipe(
    ...
    map((thContainer: HTMLElement) => ({
      ...
      thumbHeight: parseInt(
        window.getComputedStyle(document.documentElement).getPropertyValue('--thumbnail-height'),
        10
      )
    }))
  );
  ...
  prevNavigationDisabled$ = combineLatest([this.navigationDisabled$, this.currentPageIndex$]).pipe(
    map(([navDisabled, currentIndex]) => navDisabled || currentIndex === 0)
  );

  nextNavigationDisabled$ = combineLatest([this.navigationDisabled$, this.currentPageIndex$]).pipe(
    withLatestFrom(this.evtModelService.pages$),
    map(([[navDisabled, currentIndex], pages]) => navDisabled || currentIndex === pages.length - 1)
  );

  pageSliderOptions$ = combineLatest([this.navigationDisabled$, this.evtModelService.pages$]).pipe(
