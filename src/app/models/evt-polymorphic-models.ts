...
class ViewerController<T extends Record<string, ViewerSource>> {
  ...
  constructor(classes: ViewerConstructors<T>) {
    this.factories = Object.fromEntries(Object.entries(classes).map(([key, value]) => [key, new value()])) as T;
  }
  ...
  getTileSource<K extends keyof T>(
    change: BehaviorSubject<ViewerDataInput>,
    type: string | K,
    http?: HttpClient
...
class ManifestSource {
  ...
  getTileSource(change: BehaviorSubject<string>, http: HttpClient): Observable<OsdTileSource[]> {
    return change.pipe(
      ...
      map(manifest =>
        manifest.sequences // get the resource fields in the manifest json structure
          .map(seq => seq.canvases.map(canv => canv.images).reduce((x, y) => x.concat(y), []))
...
class XMLSource {
  ...
  getTileSource(change: BehaviorSubject<XMLImagesValues[]>): Observable<OsdTileSource[]> {
    return change.pipe(map(value => value.map(this.buildTileSource)));
  }
